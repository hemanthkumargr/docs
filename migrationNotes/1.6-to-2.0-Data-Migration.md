...
5. **Move order-specific taxes to new model**
  - Since the tax columns on the order have gone away, you will have to determine how you want to save order-specific taxes on the fulfillment group. Since there may be more than on fulfillment group per order, you will have to determine how you want to split taxes among the fulfillment groups.  Use the records in `TMP_BLC_TAX_DETAIL` where `ORDER_ID` is not null.  You can then create `BLC_TAX_DETAIL` records with these.  However, you will have to determine how to associate them with fulfillment groups in `BLC_FG_FG_TAX_REF`.

6. **Optionally, drop the temporary table - `DROP TABLE TMP_BLC_TAX_DETAIL;`**
