...
Please review all the possible properties for EmailInfo:

| Property | Description | Programmatic or Spring config |
...
public interface MyEmailWebService {
	...
	public abstract void sendOrderConfirmation(Date orderDate, String orderId, String emailAddress) throws IOException;
...
public class MyEmailServiceImpl implements ApplicationContextAware, MyEmailWebService {
	...
	private EmailInfo getOrderConfirmationEmailInfo() {
...
{code}
<bean id="blCheckoutWorkflow" class="org.broadleafcommerce.workflow.SequenceProcessor">
	<property name="processContextFactory">
		<bean class="org.broadleafcommerce.checkout.service.workflow.CheckoutProcessContextFactory"/>
	</property>
	<property name="activities">
		<list>
			<bean class="org.broadleafcommerce.checkout.service.workflow.PricingServiceActivity"/>
...
In this workflow configuration, we've changed the last activity to a new custom activity we've developed. Please refer to [Checkout Workflow Configuration|Checkout Workflow Configuration] for more information.

We also need to implement our new custom activity:
...
The only changes we made were to set the sendEmailReliableAsync property to false and remove the sendAsyncPriority property (sendAsyncPriority is only required for asynchronous emails).

h3. Non-Template Emails
...
{code}
<bean id="orderConfirmationEmailInfo" class="org.broadleafcommerce.email.service.info.EmailInfo" parent="baseEmailInfo" scope="prototype">
